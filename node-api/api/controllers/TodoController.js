...
export function addTodo(req, res) {
  const data = {
    ...
    date:  checkDate(req.body.date) ? req.body.date : (new Date()).toISOString().slice(0, 10),
    done:  req.body.done || false,
    alert:  req.body.alert
  };

  TodoSchema
    .create(data, function(err, aTodo){
      let response = {status: 520, message: {status: 520, message: "Unknown Error"}};
      if(err) {
        console.error(err);
        response = {status: 400, message: {status: 400, message: err.message}}
...
export function deleteTodo(req, res) {
  ...
  TodoSchema
    .findByIdAndRemove(id, function(err, aTodo) {
      let response = {status: 520, message: {status: 520, message: "Unknown Error"}};
      if(err){
        console.error(err);
        response = {status: 400, message: {status: 400, message: err.message}}
...
export function getTodoById(req, res) {
  ...
  TodoSchema
    .findById(id, function(err, aTodo) {
      let response = {status: 520, message: {status: 520, message: "Unknown Error"}};
      if(err){
        console.error(err);
        response = {status: 400, message: {status: 400, message: err.message}}
